...
export const orderRepository = {
  async createOrder(payload) {
    ...
    try {
      ...
      if (!zone) {
        throw new Error("Zona e dergeses nuk ekziston.");
      }
      ...
      for (const item of payload.items) {
        ...
        if (!product) {
          throw new Error(`Produkti ${item.product_id} nuk ekziston.`);
        }
        if (Number(product.is_active || 0) !== 1) {
          throw new Error(`Produkti ${product.title} nuk eshte aktiv.`);
        }
        if (Number(product.stock_qty || 0) < Number(item.quantity || 0)) {
          throw new Error(`Produkti ${product.title} nuk ka stok te mjaftueshem.`);
        }
